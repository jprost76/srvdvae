...
class LRHR_PKLDataset(data.Dataset):
    def __init__(self, opt):
        ...
        # todo: this is very slow (~15min) when using nn.DistributedDataParallel(), and we have to set n_worker=0
        ...
        #     cv2.imwrite("/cluster/work/cvl/jinliang/log/srflow_experiments/ICCV21/baseline_results/CelebA_HR_8X/{}.png".format(i), img[:,:,[2, 1, 0]])
        #     img = self.lr_images[i]
        #     img = np.transpose(img, [1,2,0])
        #     cv2.imwrite("/cluster/work/cvl/jinliang/log/srflow_experiments/ICCV21/baseline_results/CelebA_LR_8X/{}.png".format(i), img[:,:,[2, 1, 0]])
