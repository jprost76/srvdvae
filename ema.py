...
class EMA(pl.Callback):
    ...
    def get_state_dict(pl_module: pl.LightningModule):
        ...
        """
        # only keep trainable lr encoder weights 
        keep_key = lambda k: "encoder_y" in k or "enc_y" in k
        full_state_dict = pl_module.state_dict()
        state_dict = {k:full_state_dict[k]  for k in full_state_dict if keep_key(k)}
    ...
    def on_save_checkpoint(
        ...
        self.state_dict = {"ema_state_dict": self.ema_state_dict, "_ema_state_dict_ready": self._ema_state_dict_ready}
